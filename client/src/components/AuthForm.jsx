        ...
        <div className="text-center py-2">
          ...
          {devResetUrl && (
            <div className="mb-5 text-left bg-amber-50 border border-amber-200 rounded-lg p-3">
              ...
              <a href={devResetUrl} className="text-xs text-brand-teal break-all underline">{devResetUrl}</a>
            </div>
          )}
          <button
            onClick={() => { setMode('login'); setForgotSent(false); setDevResetUrl(null); setError(null); }}
            className="text-brand-teal hover:text-brand-teal-dark text-sm font-medium transition-colors">
            ← Back to sign in
          </button>
        </div>
      );
