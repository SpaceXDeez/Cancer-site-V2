...
export default function ChatSidebar({
  ...
  onNewChat, onSelectChat, onDeleteChat, onRenameChat, onOpenProfile, onLogout, onDeleteAccount, onSetView, onCloseSidebar,
}) {
  ...
  return (
    <aside className={`w-64 bg-slate-900 flex flex-col h-full flex-shrink-0 fixed inset-y-0 left-0 z-50 md:relative md:z-auto transition-transform duration-300 ease-in-out md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="px-3 py-3 border-t border-slate-700/60 space-y-1">
        ...
        {confirmDelete ? (
          <div className="px-3 py-2 rounded-lg bg-red-950/60 border border-red-800/50">
            <p className="text-xs text-red-300 mb-2">Delete account and all data? This cannot be undone.</p>
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-slate-600 hover:text-red-500 hover:bg-slate-800 transition-colors text-xs"
          >
